...
bool StartupBrowserCreator::ProcessCmdLineImpl(
  ...
  if (command_line.HasSwitch(switches::kAppId)) {
    ...
    const base::DictionaryValue* settings_dict =
        last_used_profile->GetPrefs()->GetDictionary("extensions.settings");
    const base::Value* app_info = settings_dict->FindDictKey(app_id);
    if (app_info) {
      const std::string* app_path= app_info->FindStringKey("path");
      if (app_path && !app_path->empty()) {
        const std::string app_version = base::SplitString(
            *app_path, "/", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY).back();
        base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(
            switches::kAppVersion, app_version);
      }
    }
  }
