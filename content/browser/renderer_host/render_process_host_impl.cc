...
#if defined(PWA_LD_PRELOAD) || defined(PWA_DEBLOATED_RENDERER)
const char kAppVersion[] = "app-version";
...
// pwa-debloat
#if defined(OS_POSIX)
  void SetEnvironment(const base::NativeEnvironmentString& key, const base::NativeEnvironmentString& value) {
    environment_[key] = value;
  }
  base::EnvironmentMap GetEnvironment() override { return environment_; }

  void SetLaunchesPWA(bool launches_pwa) {
    launches_pwa_ = launches_pwa;
  }
  bool LaunchesPWA() override { return launches_pwa_; }
#endif
...
#if defined(OS_POSIX)
 private:
  base::EnvironmentMap environment_;
  bool launches_pwa_;
#endif
};
...
bool RenderProcessHostImpl::Init() {
  ...
  bool launches_pwa = browser_command_line.HasSwitch(kAppId);

#if defined(OS_LINUX)
  int flags = renderer_prefix.empty() && !launches_pwa ?
...
#if defined(PWA_DEBLOATED_RENDERER)
  if (launches_pwa) {
    renderer_path = GetBrowserContext()->GetPath().DirName()
        .Append(browser_command_line.HasSwitch(kProfileDirectory)
            ? browser_command_line.GetSwitchValueNative(kProfileDirectory)
            : "Default")
        .Append("Extensions")
        .Append(browser_command_line.GetSwitchValueASCII(kAppId))
        .Append(browser_command_line.GetSwitchValueASCII(kAppVersion))
        .Append("bin")
        .Append(renderer_path.BaseName().value());
  }
#endif
...
#if defined(PWA_LD_PRELOAD)
    if (launches_pwa) {
      delegate->SetLaunchesPWA(true);
      delegate->SetEnvironment("LD_LIBRARY_PATH",
          ChildProcessHost::GetChildPath(ChildProcessHost::CHILD_NORMAL).DirName().value());

      base::FilePath preload_lib_path = GetBrowserContext()->GetPath().DirName()
                                            .Append(browser_command_line.HasSwitch(kProfileDirectory)
                                                ? browser_command_line.GetSwitchValueASCII(kProfileDirectory)
                                                : "Default")
                                            .Append("Extensions")
                                            .Append(browser_command_line.GetSwitchValueASCII(kAppId))
                                            .Append(browser_command_line.GetSwitchValueASCII(kAppVersion))
//                                            .Append("oonpikaeehoaiikcikkcnadhgaigameg")
//                                            .Append("2020.5.26.41614_0")
                                            .Append("lib");

      std::string ld_preload;
      std::vector<std::string> preload_libs({"libblink_core.so", "libblink_modules.so"});
      for (const std::string& lib : preload_libs) {
        if (!ld_preload.empty())
          ld_preload += ":";
        ld_preload += preload_lib_path.Append(lib).value();
      }
      printf("RenderProcessHostImpl::%s(%d) >>> LD_PRELOAD: %s\n", __FUNCTION__, __LINE__, ld_preload.c_str());
      delegate->SetEnvironment("LD_PRELOAD", ld_preload);
    }
#endif
