...
#if defined(OS_LINUX) && defined(PWA_DEBLOAT)
#include <sys/wait.h>
extern char **environ;
#endif
...
void ServiceWorkerRegisterJob::OnStoreRegistrationComplete(
  ...
  ssize_t count = readlink("/proc/self/exe", &buffer[0], buffer_size);
  base::FilePath exe_path(std::string(&buffer[0], count > 0 ? count : 0));
  base::FilePath dev_script_path = exe_path.DirName().DirName()
                                       .DirName().DirName().DirName().DirName()
                                       .Append("dev.py");

  //std::vector<std::string> cmds({dev_script_path.value(), "run", "debloating"});
  //std::string debloating_command = base::JoinString(cmds, " ");
  ...
  if (pid == 0) {
    ...
    for (int i = 0; environ[i]; i++) {
      if (strstr(environ[i], "LD_PRELOAD="))
        continue;
      env.push_back(const_cast<char*>(environ[i]));
    }
    env.push_back(nullptr);

    std::vector<char*> args;
    args.push_back(const_cast<char*>("dev.py"));
    args.push_back(const_cast<char*>("run"));
    args.push_back(const_cast<char*>("debloating"));
    args.push_back(nullptr);

    execve(dev_script_path.value().c_str(), &args[0], &env[0]);
    exit(EXIT_FAILURE);
  }

//  int state;
//  if (waitpid(pid, &state, 0) == -1)
//    printf("%s(%d) Something goes wrong in waiting for child process.\n", __FUNCTION__, __LINE__);
#endif
